            ...
            {(content.pricing.plans as PricingPlan[]).map((plan, i) => {
              ...
              return (
                <motion.div
                  ...
                  className={`
                    relative flex-shrink-0 w-72 sm:w-auto snap-center rounded-2xl p-7 flex flex-col gap-5
                  ...
                  <ul className="flex flex-col gap-3 flex-1">
                      ...
                      <li key={j} className="flex items-start gap-2.5">
                        <Check
                          className={`w-4 h-4 mt-0.5 flex-shrink-0 ${
                            feat.startsWith("BONUS") || feat.startsWith("BONO")
                              ? "text-[#C9A84C]"
                              : "text-emerald-400"
                          }`}
                        />
                    ...
                    {isQuickWin && (
                      <li className="flex items-start gap-2.5 mt-1 pt-3 border-t border-[rgba(255,255,255,0.06)]">
                        <X className="w-4 h-4 mt-0.5 flex-shrink-0 text-[#6B6B7A]" />
                  ...
                  <ul className="flex flex-col gap-2 border-t border-[rgba(255,255,255,0.06)] pt-4">
                    {addons.map((addon, k) => (
                      <li key={k} className="flex items-start gap-2">
                        <span className="w-1 h-1 rounded-full bg-[#6B6B7A] mt-2 flex-shrink-0" />
                ...
                <div key={i} className="flex items-center justify-between py-3">
                  ...
                  <span
                    className={`font-sans text-sm font-semibold ${
                      row.value === "Incalculable" || row.value === "Incontable"
                        ? "text-[#6B6B7A] italic"
                        : "text-[#DC2626]"
                    }`}
                  >
            ...
            <div className="relative pl-6 flex flex-col gap-0">
              ...
              {noShowSteps.map((step, i) => (
                <div key={i} className="relative flex items-start gap-4 pb-5 last:pb-0">
                  <div className="absolute -left-4 top-1 w-3 h-3 rounded-full border-2 border-[#C9A84C] bg-[#0F0F14] flex-shrink-0" />
